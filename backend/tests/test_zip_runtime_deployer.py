...
# A resolved lock the way `uv pip compile --generate-hashes` writes one.
FAKE_LOCK = (
    "# autogenerated by uv\n"
...
def fake_pip_ok(cmd, **kwargs):
    """Stands in for both subprocesses build_zip drives: the uv resolve (writes a
    lock at `-o`) and the pip install (populates `-t`)."""
    if "compile" in cmd:
        Path(cmd[cmd.index("-o") + 1]).write_text(FAKE_LOCK, encoding="utf-8")
        fake_pip_ok.compile_cmd = cmd
...
def test_build_zip_contains_expected_files(tmp_path: Path):
    ...
    # the lock ships inside the artifact, so what ran carries what it was built from
    ...
    # the install must verify against the lock, not re-resolve the declared ranges
    assert "--require-hashes" in cmd
    assert "-r" in cmd


def test_build_zip_resolves_a_hashed_lock_for_the_deploy_target(tmp_path: Path):
    """Resolving for this host and installing for aarch64 would produce a lock
    that does not describe the artifact."""
    build_zip(
        "print('agent')",
...
def test_build_zip_reports_the_locked_package_count(tmp_path: Path):
    ...
    build_zip(
        ...
        tmp_path / "build",
        pip_runner=fake_pip_ok,
        on_lock_ready=seen.append,
    )
    assert seen == [2]  # FAKE_LOCK pins two packages


def test_build_zip_fails_when_the_resolve_fails(tmp_path: Path):
    """No fallback to the unchecked install — a control that reverts to the
    unverified path when it cannot verify is decoration."""

    class Failed:
        returncode = 1
        stderr = "no solution found for strands-agents"
        stdout = ""

    def failing_compile(cmd, **kwargs):
        return Failed()

    with pytest.raises(RuntimeError, match="hashed lockfile"):
        build_zip(
            "print('agent')",
            ["strands-agents[otel]>=1.0,<2"],
            tmp_path / "build",
            pip_runner=fake_pip_ok,
            compile_runner=failing_compile,
        )
