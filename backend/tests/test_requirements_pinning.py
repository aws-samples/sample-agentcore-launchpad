...
class TestResolvePins:
    ...
    def test_extras_are_reattached(self):
        """`uv pip compile --no-deps` drops extras from its output, so
        `botocore[crt]` would silently become plain `botocore` and install
        different code."""
        out = resolve_pins(
            ["botocore[crt] >= 1.35.0"],
    ...
    def test_an_entry_missing_from_the_output_is_an_error(self):
        ...
        with pytest.raises(ValueError, match="not present in the resolver output"):
            resolve_pins(["wanted>=1"], runner=self._runner("something-else==1.0.0\n"))
