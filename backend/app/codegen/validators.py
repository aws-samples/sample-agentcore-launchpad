...
Ported from strands_studio_ui ``backend/codegen/validators.py`` (origin/main).
Stages (short-circuit on first failing stage):
  1. AST contract validation (in-process, no execution risk)
  2. ruff check --select E9,F (hard errors only) via uvx
...
The one launchpad change from upstream is the import-smoke interpreter: the lean
control-plane backend env has no strands, so the smoke test runs against
``settings.studio_exec_python`` (the dedicated exec venv). When that interpreter
is absent the stage is skipped-with-note (AST + ruff still gate the code) rather
than failing every fix.
"""
...
async def run_import_smoke(workspace: Path) -> list[ValidationIssue]:
    ...
    Runs against ``settings.studio_exec_python`` (the exec venv carries strands),
    with cwd=workspace so generated_agent is importable. When that interpreter is
    missing the stage is skipped-with-note (returns no issues): the lean
    control-plane env cannot import strands, so failing here would block every
    fix even though AST + ruff already gate the contract.
    """
    exec_python = config.get_exec_python()
    if not os.path.isfile(exec_python):
        logger.warning(
            "import smoke skipped: exec interpreter %s not found "
            "(run scripts/setup_exec_env.sh to enable this stage)",
            exec_python,
        )
        return []
    ...
    try:
        ...
        return [
            ValidationIssue(
                stage="import",
                message=(
                    f"exec interpreter {exec_python} not runnable; "
                    "cannot run import smoke test"
                ),
            )
        ]
    except TimeoutError:
