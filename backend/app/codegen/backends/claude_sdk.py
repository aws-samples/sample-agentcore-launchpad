...
Ported from strands_studio_ui ``backend/codegen/backends/claude_sdk.py``
(origin/main); only the import paths changed (``codegen`` -> ``app.codegen``).
Runs a headless Claude Code session (via claude-agent-sdk, Bedrock auth) inside
a temporary workspace to produce/repair generated_agent.py.
"""
...
from app.codegen.backends.base import CodingAgentBackend, GenerationError, GenerationTask
            ...
            if isinstance(message, AssistantMessage):
                ...
                if message.is_error:
                    raise GenerationError(
                        f"Claude agent session ended with error: "
