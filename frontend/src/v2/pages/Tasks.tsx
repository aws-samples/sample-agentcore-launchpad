 ...
 * states: `new` (2-step wizard, `from=` to copy a task, `dataset=` to preselect
 * one) and `detail` (`kind=run|online&id=`).
 */
export function V2Tasks() {
  ...
  if (view === "new") return <TaskWizard key={`${params.get("from")}:${params.get("dataset")}`} />;
