...
本次截图里就同时含实验室之外的活动，以及本实验从 sonnet-4-6 换到 sonnet-5 前后的两种模型。*
...
本次这条 17.9s / 46 spans 的 trace 跑在本实验选定的 `global.anthropic.claude-sonnet-5` 上。一次"有知识库的问答"
可以拆成下面这些步骤：
...
从这条 trace 可以判断延迟主要来自哪里：17.9 秒里有 15.8 秒在 Agent 循环，其中四次模型调用占
12.1 秒，两次 KB 检索占 1.9 秒。要优化延迟，应先减少循环轮次，而不是先换向量库。
