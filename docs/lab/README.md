...
第 09 章让优化器分析 trace，也找出同样的问题，再用改进后的提示词做 A/B；第 10 章用金丝雀把
改进版本灰度到真实流量上。每一步都有日志、trace、评估分数或判定结果可供核对。
...
所有截图、日志、分数和耗时都取自 **2026-07-26 的一次完整实跑**，不是示意图。

平台默认模型现在是 `global.anthropic.claude-sonnet-5`，第 02/03/07/08 章涉及模型的截图与
瀑布图都已在 sonnet-5 上重新采集。第 02/03 章里那两段**首次创建**的事件流保留原样，它们录于
默认还是 `claude-sonnet-4-6` 的时刻，不做改写；两章各自紧跟一段切换到 sonnet-5 后的真实日志。
个别未实跑的步骤会显式标注「本次未实跑」并说明原因，共三处：

| 未实跑的部分 | 原因 |
|---|---|
| 第 01 章 `make bootstrap` | 实验环境早已完成引导；命令与产物清单按 `docs/setup.zh-CN.md` 与现存 `config/launchpad.yaml` 校对 |
| 第 10 章 50/50 与 1/99 两档 | 90/10 档判定为 `insufficient-data`，门禁按设计阻断后续放量 |
| 第 11 章 Registry 导入 Gateway 记录 | 只跑了只读 `预览`；不执行导入以免在共享演示账号里留下不可恢复的目录改动（`DEPRECATED` 是终态） |

本次实跑还遇到一个产品缺陷：**方式A 容器部署成功但调用失败**（依赖漂移导致容器启动即崩）。
该缺陷**已于同日修复并真机复验**（重新发布 → Runtime v2 → 调用 5.5 秒正常返回 → 遥测正常落地）。
当前代码已经包含这项修复。现象与修复见[第 05 章末](05-chat-memory.md#关于容器-agent-调用失败本次实测)，
完整根因与验证记录见
[docs/issues/2026-07-26-container-otel-events-import.md](../issues/2026-07-26-container-otel-events-import.md)。
仍有一条未关闭的已知问题：**部署全绿不代表容器能起来**。平台没有部署后探活调用，
