...
>
> **前置条件**：完成[第 01 章](01-environment.md)，服务健康 7 项全绿。
>
...
平台有三种创建方式，能力**不等价**。后面章节要用的高级能力对方式有硬性要求，先记住这张表
...
| 挂载知识库（托管 RAG） | ✅ **仅此方式** | ❌ | ❌ |
| 挂载 Registry 技能 / MCP | ✅ | ✅ | 模板内置工具 |
| 可被评估（第 08 章） | ✅ | ✅ | ✅ |
| **配置包 A/B 实验**（第 09 章） | ❌ 明确不支持 | ❌ | ✅ **仅此方式** |
| **Runtime 金丝雀**（第 10 章） | ❌ | ❌（后续支持） | ✅ |

> 为什么 Harness 不能做配置包 A/B：它背后的 Runtime 是「被 Harness 托管」的，
...
*图 2-2：配置页。注意底部提示：Config-bundle 契约与 OTEL(ADOT) 埋点已自动注入——
这是第 07 章能看到追踪、第 09 章能做 A/B 的原因。*
...
> 📌 **记下你的 `id`**，后面章节记作 `<ASSISTANT_ID>`；本次实验里它是
...
- **异步可恢复**：`POST /api/agents` 立刻返回 `202` 和 `job_id`，部署在后台线程跑。
  后端重启时 `resume_pending_jobs()` 会从第一个未成功的阶段继续——所以每个阶段都是幂等的。
...
| 想改提示词 | 编辑后需重新发布 | 列表行「编辑」→ 重新发布；**注意已有会话会继续用旧版本，验证要开新会话** |
