...
第01章 环境          bootstrap → 共享基础设施 + AgentCore 单例
   ↓
第02章 部署          方式C ZIP 通道 → lab-fund-assistant
第03章 部署          方式B Harness → lab-fund-advisor
                     方式A 容器   → lab-fund-packager（CodeBuild）
   ↓
...
第06章 集成（可选）  /v1 + API Key：同步与流式，与控制台同一条链路
   ↓
第07章 观测          仪表盘 / 会话还原 / trace 瀑布图：定位模型调用、检索与延迟
   ↓
第08章 评估          带真值的数据集 + 自定义评审 → 区分自洽与事实接地
   ↓
第09章 优化          优化器读 trace，也发现"编造数字"问题 → 配置包 A/B 50/50 → 判定
第10章 金丝雀        目标金丝雀：真实生产流量分档放量 + 每档证据门禁
   ↓
第11章 治理          Gateway 纳管（只加标签）→ Cedar LOG_ONLY 策略 → 决策证据 → 不可变审计
```

可以用部署日志、对话结果、trace、评估分数、实验判定和审计记录逐项核对整套流程。
...
| 资源 | 标识 | 清理方式 |
|---|---|---|
| Agent `lab-fund-assistant`（zip runtime） | 在 `02 Agent 管理` 查看 Runtime ID | `02 Agent 管理` 列表行 → `删除` |
| Agent `lab-fund-advisor`（harness） | 在 `02 Agent 管理` 查看 Harness ID | 同上 |
| Agent `lab-fund-packager`（container） | 在 `02 Agent 管理` 查看 Runtime ID | 同上 |
...
| 资源 | 标识 | 清理方式 |
|---|---|---|
| 托管知识库 `lab-fund-kb` | 在 `04 知识库` 查看 KB ID | 先在 `lab-fund-advisor` 编辑里取消勾选并重新发布，再到 `04 知识库` 详情点 `删除` |
| S3 数据源对象 | `s3://launchpad-artifacts-<ACCT>-us-west-2/kb/<KB_ID>/` | 随 KB 删除清理；S3 对象可手动确认 |
...
| 记录 | id / 状态 | 清理方式 |
|---|---|---|
| `lab-fund-disclaimer`（技能） | 在 `03 注册中心` 查看 | `03 注册中心` 详情 → `删除` |
| `lab-fund-assistant`（A2A，自动登记） | 在 `03 注册中心` 查看 | 删 Agent 时一并处理，或手动删记录 |
| `lab-fund-advisor`（A2A） | 在 `03 注册中心` 查看 | 同上 |
| `lab-fund-packager`（A2A） | 在 `03 注册中心` 查看 | 同上 |
...
| 资源 | 标识 | 清理方式 |
|---|---|---|
| 本地数据集 `lab-fund-dataset` | 在 `?view=datasets` 查看 | `?view=datasets` 选中 → `删除` |
| AWS 数据集快照 | 在数据集详情查看 | 云端副本只能删除（不可编辑） |
| 自定义评估器 `fund_fact_grounding` | 在 `?view=evaluators` 查看 | `?view=evaluators` 选中 → `删除` |
| 回放产生的 Runtime 会话与遥测 | 每个场景一个 session | CloudWatch 日志按保留期自动过期 |

### 实验与金丝雀（**必须清理**）

| 资源 | 标识 | 清理方式 |
|---|---|---|
| 配置包 control / treatment | 在实验详情查看 | 实验详情 → `清理` |
| 实验专属 Gateway + target | 在实验详情查看 | 同上 |
| A/B 测试 | 在实验详情查看 | 同上 |
| 在线评估配置 | 在实验详情查看 | 同上 |
...
| 资源 | 标识 | 清理方式 |
|---|---|---|
| 第 06 章签发的 API Key | 在对话页 API 密钥面板查看 | 切到 `已停用`（或删除） |
| Cedar 策略 `lab_readonly_tools` | 在 `09 治理` 查看 | 策略列表 → 删除（它是 `LOG_ONLY`，留着也不拦流量） |
...
如果环境还要用于演示，可以保留 `lab-` Agent、知识库、Registry 记录、数据集和评估器。
实验和金丝雀的中间资源必须清理；它们会占用共享网关互斥锁，见第 09/10 章各自的 `CLEANUP` 阶段。
...
| 来源 | 怎么检查 |
|---|---|
| Bedrock 模型调用（对话 + 评估回放 + 评审 + A/B 流量 + 优化器） | 在可观测页按模型查看 token 与估算成本 |
| 知识库（向量库 + embedding + 检索） | 检查索引、embedding 和检索调用 |
