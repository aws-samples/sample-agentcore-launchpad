...
调用 `max_tokens: 65536` 与 `reasoning_effort: high`。之后管理员可通过面板的**“配置”**对话框（或同一安装
API 请求体）修改已存设置：模型来源/ID、单次模型调用最大输出 token（即 Harness 的
...
核验；不会替你创建任何知识库）。保存只提交改动的字段并运行标准更新任务；成员只能只读查看同样的值。修复与技能包
更新会保留已存设置——早期构建安装的预置会保留其模型与提示词，直到你保存更改或点击“使用预置默认值”。
“修复/更新”就地重新发布；“卸载”排入拆除任务——预置显示“卸载中”，并在 Harness 与角色
...
重命名该 Agent；预置绝不接管它。
预置的带版本 Skill 也可以作为独立记录列入 Registry：预置进入运行中后，管理员在面板中点击
**“注册 SKILL”**（或 `POST /api/system-agents/<key>/skill-registration`）。这会让一条新的
`AGENT_SKILLS` 记录指向已发布在 `system-skills/` 下的版本目录（不上传任何内容，也不重新发布
Harness），并提交审核；只有当你**在 Registry 中批准**之后，其他 Agent 才能挂载它。重复该操作是
空操作（“核验 SKILL 记录”）；技能包更新后它会把记录前滚到新版本，需要再次批准。该记录带有“系统”
标签：没有人能在控制台编辑、重新导入或删除它，成员可以查看与挂载，管理员负责其审批；卸载预置会
保留该记录。用本构建执行的安装与修复会在部署的 register 阶段自动注册 Skill。
