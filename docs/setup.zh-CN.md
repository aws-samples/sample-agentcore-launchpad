...
只有两个角色。`admin` 拥有整个控制台。`member` 可以浏览智能体、注册表记录与知识库,
与智能体对话、调用,使用检索 playground,以及查看可观测性、记忆、评估与治理。在此
之上,有一小组能力**默认授予成员、可在用户管理控制台按用户撤销**:智能体生命周期
(部署、导入、删除、转换)以及发起评估/洞察运行。

其余会执行代码、改变已部署或云端状态、签发凭证、或改变治理策略的操作仍仅限管理员
—— Studio 画布、注册表的注册/编辑/导入、知识库变更、API Key、
