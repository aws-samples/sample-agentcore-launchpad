---
title: "01A 可选：Self-paced 自有 AWS 账号与本地开发机"
weight: 15
---
...
> `global.anthropic.claude-sonnet-5` ,`openai.gpt-5.6-sol`等 或回退模型 `global.amazon.nova-2-lite-v1:0`，
> 同时允许创建本章列出的共享资源。
>
> 确认服务健康后，直接进入[第 02 章](../02-deploy-runtime)。
>
> **预计耗时**：首次约 15–20 分钟，其中 `make bootstrap` 约 8–12 分钟；已引导过的环境约 2 分钟。
>
> **费用和清理**：本章创建的资源由你的 AWS 账号计费，Workshop Studio 不会代为删除。
> 完成实验后请按[第 12 章](../12-wrapup-cleanup)清理。
...
AWS CLI v2 的安装步骤见[第 01 章 1.1 节](../01-environment#11-在个人电脑上安装-aws-cli-与-session-manager-plugin)，
本章走本地路径，不需要 Session Manager Plugin。

如果本机还没有源码：

```bash
git clone --depth 1 --branch v0.0.3 https://github.com/xiehust/agentcore_launchpad.git
...
- [ ] 创建 Agent 时会把默认的 `Bedrock Mantle` 改成 `Bedrock`；首选
      `global.anthropic.claude-sonnet-5`，不可用时回退到 `global.amazon.nova-2-lite-v1:0`
...
| 「服务健康」某项为红或未就绪 | bootstrap 未完成、区域错误或权限不足 | 核对身份和区域后重跑 `make bootstrap`；仍失败时查[排障文档](https://github.com/xiehust/agentcore_launchpad/blob/main/docs/troubleshooting.zh-CN.md) |
