...
| 会话 | 确认 `lab-quota-assistant` 与 `lab-quota-advisor` 的测试会话都出现 |
...
```
<SESSION_ID_A>  lab-quota-assistant  <TRACE_COUNT> traces  <LLM_COUNT> LLM  <TOKENS> tokens  <EST_COST>
...
- [ ] 会话列表能看到自己的 `lab-quota-advisor` 与 `lab-quota-assistant` 会话
