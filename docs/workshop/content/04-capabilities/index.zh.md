...
> 下载并准备好[AgentCore 配额文档](../static/assets/bedrock-agentcore-limits.md)。第 08 章的评估真值
> 也来自这份文档。
>
...
> `lab-quota-assistant` 保持不挂知识库，继续作为后续章节的基线。
...
```
<ASSISTANT_RECORD_ID>   lab-quota-assistant   A2A
...
- [ ] `lab-quota-advisor` 的 spec 里同时有 `skills` 与 `knowledge_bases`，版本高于重新发布前
