...
**确定性检查**在本地基于 agent 的实际回复运行（绝不看提示词、描述或真值），给出 `pass` / `fail` / `error` / `inconclusive`：缺少回复（调用失败、运行被停止）记为 `error`，未满足的 `depends_on` 使依赖它的检查变为 `inconclusive`，运行级 `check_status` 采取失败优先的汇总（`fail` > `error` > `inconclusive` > `pass`；未产出全部计划检查的运行不可能是 `pass`；未声明检查时为 `none`）。它们**不是** AgentCore 评估器：批量评审独立地为同一批会话打分，代码评估器的 Lambda 只能看到单个会话的 span，工具调用 / trace 断言属于未来的代码评估器而非这个文本级执行器。运行页在评审结果下方以独立的「流程结果」面板渲染该台账（会话及其别名、角色、运行时会话链接与状态；每条检查及其结论与截断后的证据），可观测性中流程会话的对话记录使用持久化的合成角色而非裸 `default` 读取 Memory。数据集编辑器在表单编辑中保留所有未知的 scenario 键（包括这段块），以只读预览展示流程并禁用增删轮次，修改则通过按 scenario 的 JSON 编辑完成；实验的流量阶段会拒绝这类数据集（`experiment.dataset_unsupported`），而不是把它压平成首轮提示词。
