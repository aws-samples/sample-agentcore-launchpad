...
- **评估器选择只能是全局的；按黄金测试的子集映射会被拒绝而不是假装实现**。批量运行把同一评估器列表应用到每个
  ...
  = 全部，或改为 runner 检查 / 人工义务）。读取参考的全局评估器要求**每个**场景（TRACE 级别还要求每一轮）都携带该参考，
...
- **首次初始化会改变 RevisionId：只审阅，不自动重钉（SE-049）**：`CreateFunction` 在函数仍在配置时即返回
  ...
  `MasterArn` 或平台未知的成员都是差异），响应的每个成员必须由请求固定或属于文档默认值。依赖与记录快照重新比对（信任策略、
  内联策略、标签、保留期 —— `ready` 账本状态不能背书，worker 会跳过 ready 依赖），资源策略只有 `NotFound` 才证明不存在。记录
  审阅的条件 UPDATE 以当前所有者、已批准计划行、钉住的 Workspace 以及审批人与审阅人的活跃管理员行为谓词，并在主机锁内重新解析
  调用者。核验状态持久化为 `reviewed_baseline`，恢复的 worker 在首次变更前立即重新校验（配置 + 标签、依赖、清单、策略不存在、
...
## 可执行的多角色 / 多会话评估 scenario

普通的 predefined scenario 在**同一个**运行时会话、裸 `default` 角色下顺序回放其 `turns`，无法验证记忆隔离与会话新鲜度类的黄金测试（「A 在某会话告诉 agent 一个秘密，B 不得知晓」「A 新开一个会话，agent 仍应记得」）。scenario 可以通过标准 `metadata` 内部一段带版本号的命名空间块 —— `metadata.launchpad_execution` —— **显式选择**进入流程执行器；SDK 的 `Scenario.metadata` 与 AWS 数据集 schema（自由 JSON 示例文档）都会原样保留它。没有该键的一切保持原有线上行为；模拟 persona、旧式 prompt 条目与 A2A agent 不能启用（`backend/app/evaluation/execution.py`）。

```json
"metadata": {"launchpad_execution": {
  "version": 1,
  "repeat": 2,
  ...
  "checks": [{"id": "seed", "type": "contains", "turn": 0, "text": "amber"},
             {"id": "recall", "type": "contains", "turn": 1, "text": "amber", "depends_on": ["seed"]},
             {"id": "no_leak", "type": "not_contains", "turn": 2, "text": "amber", "depends_on": ["seed"]}]
}}
```
...
|---|---|
| `version` | 必须为 `1` |
| `repeat` | 整数 1–5；每次都用全新的角色与会话重放**整个**流程 |
| `steps` | 每个 `turns[]` 下标恰好一个步骤（重复、缺失、越界都会被拒绝），按列表顺序执行；`actor` / `session` 是**别名**（`^[A-Za-z][A-Za-z0-9_-]{0,31}$`，每个 scenario ≤ 10 个角色、≤ 20 个会话），绝不是原始 id；一个会话别名只属于一个角色别名 |
| `checks` | 可选，≤ 20 条：`{id, type: exact\|contains\|not_contains, turn, text（≤ 2000 字符）, depends_on?, case_sensitive?（默认 false）}`；`depends_on` 只能引用更早声明的检查（无环）；没有正则、表达式、代码或 LLM 评审 |
| 未知键 | 块内任何位置都会被拒绝；`version` 必须是整数 `1`（不接受布尔/浮点/字符串），别名做完整匹配，出现在条目顶层或非对象 `metadata` 中的 `launchpad_execution` 会被拒绝而不是被静默忽略 |

**身份。** 对每个（workspace、agent、运行、scenario、重复次数、角色别名）平台生成一个合成记忆角色 —— `memory.scoped_actor(agent_id, "eval__<ws>__<run>__<scenario>__r<k>__<alias>")`，因此按 agent 的记忆分区与可观测性的角色探测都照常适用 —— 并为每个（重复次数、角色别名、会话别名）生成一个全新的运行时会话 id。同一角色别名跨其各会话别名保持同一身份；不同别名绝不共享；两次运行或两次重复也绝不共享角色。Harness 步骤以 `InvokeHarness.actorId` 传入，runtime 步骤以载荷 `actor_id` 传入；封装层会回显请求的会话 id；若运行时在另一个会话 id 下应答，两个 id 都会被持久化（`requested_session_id` / `returned_session_id`，均可链接），且流程以 `SessionDriftError` **拒绝继续** —— 跟随新 id 会把后续轮次归到错误的会话。角色 id 以完整作用域身份的 128 位 SHA-256 摘要结尾，因此截断后前缀相同的 scenario id 绝不会碰撞。A2A runtime 没有角色信封，因此针对 A2A agent 运行启用了该扩展的数据集，会在创建运行行或任何 AWS 调用之前被 `run.execution_unsupported_protocol` 拒绝。

**边界在任何调用之前校验** —— 数据集创建 / 上传 / 更新时，以及本地与云端数据集创建运行时再次校验，且**仅针对包含启用条目的数据集**（普通数据集保持原有接受行为不变）：每个启用条目都必须能解析且序列化不超过 16 000 字符（`dataset.invalid_execution`），归一化后的 scenario id 必须唯一（旧式 prompt 条目被命名为 `item_<N>`，因此与之并列的名为 `item_2` 的启用 scenario 会被拒绝 —— 执行器也按位置而非按名称分派），整个数据集展开后的总量不得超过 200 次调用与 500 个会话（`dataset.execution_limits`；API 本身对 `sessionMetadata` 的上限也是 500）。`submit_run` 在把回调放入队列**之前**深拷贝条目，排队期间编辑数据集不会改变它回放的内容。

**执行器。** `execute_run` 把启用的 scenario 分派给 `execution.run_scenario`；每次调用前与评估检查前都会读取停止标志，并且**每一步**之后都重新持久化运行行的 `execution` JSON 与 `session_ids` —— 被停止或失败的运行同样列出它实际打开的每个运行时会话。真值按**实际**会话拆分，而不是旧的按位置 zip：每个会话的 `sessionMetadata` 条目携带它回放过的 `expectedResponse` 轮次、唯一的 `testScenarioId`（`<scenario>#r<repeat>/<session alias>`）以及作为 API 字符串映射 `metadata` 的关联信息（`launchpad_scenario/repeat/actor/session`）；scenario 级 `assertions` / `expected_trajectory` 只附着在承载最后一步的会话（结果会话）上，因此隔离类的否定断言绝不会被拿去评判播种角色的会话。同一数据集里的普通 scenario 保持完全一致的旧调用形态与顺序。

**确定性检查**在本地基于 agent 的实际回复运行（绝不看提示词、描述或真值），给出 `pass` / `fail` / `error` / `inconclusive`：缺少回复 —— 调用失败、运行被停止，或回复没有文本 / 空白（该步骤记为 `empty`，不算完成）—— 即使对 `not_contains` 也记为 `error`；流程步骤要求 Runtime 解码器提供**严格文本证据**（`invoke_runtime_text(strict_text=True)`：原始的非字符串 `text` / `result` / 响应体会抛出 `RuntimeTextProtocolError` 并使该步骤失败，`null` 视为无回答，`text/plain` 响应体按文本读取，而字面文本 `"None"` / `"42"` 仍是文本 —— Chat、公共 API 与普通回放保持宽松的默认解码），未满足的 `depends_on` 使依赖它的检查变为 `inconclusive`，运行级 `check_status` 采取失败优先的汇总（`fail` > `error` > `inconclusive` > `pass`）。`pass` 还要求**整个流程完成**：所有计划调用都得到可用回复、每次重复声明的每项检查都有记录、且没有中断（停止或失败时 `interrupted: true`，包括落在最后一次调用上的停止 —— 声明的检查仍会被评估并保留，但汇总变为 `inconclusive` / `error`）。仅在未声明检查时为 `none`。证据文本使用控制台的 1 起始 `T<n>` 标签，API 下标保持 0 起始。它们**不是** AgentCore 评估器：批量评审独立地为同一批会话打分，代码评估器的 Lambda 只能看到单个会话的 span，工具调用 / trace 断言属于未来的代码评估器而非这个文本级执行器。运行页在评审结果下方以独立的「流程结果」面板渲染该台账（会话及其别名、角色、运行时会话链接与状态；每个步骤及其会话、状态与回复摘录或确切的调用错误；每条检查及其结论与截断后的证据；已声明但尚无记录的检查显示为进行中 / 已中断，绝不显示为「无检查」），可观测性中流程会话的对话记录使用持久化的合成角色而非裸 `default` 读取 Memory。数据集编辑器在表单编辑中保留所有未知的 scenario 键（包括这段块以及旧式 / persona 的来源字段），以只读预览展示流程并禁用增删轮次，修改则通过按 scenario 的 JSON 编辑完成，对表单无法无损呈现的条目（轮次级字段、结构化轮次输入）强制仅限 JSON 编辑；实验的流量阶段会拒绝这类数据集（`experiment.dataset_unsupported`），而不是把它压平成首轮提示词。
