...
`reasoning_effort`）返回 `422 system_agent.invalid_options`。部署任务持有该行期间的显式编辑返回
`409 system_agent.deploy_in_progress` 并附带该任务 ID，而不是合并到该任务上（无请求体的修复点击仍会合并），
因此并发保存绝不会被悄悄丢弃。维护
...
安装按钮一样消费 `202`/`200`/`409`/`422` 结果；取消不提交任何内容，切换 Workspace 会关闭对话框，因此迟到的
响应绝不会保存到另一个 Workspace。持久记忆保持关闭：这只关闭 AgentCore *记忆*——Launchpad 台账中的对话记录
