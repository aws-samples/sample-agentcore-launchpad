...
| `DELETE` | `/api/system-agents/{key}` | admin | `202 {agent, job_id, operation: uninstall, attempt, started, preset}` — claims the row (`uninstalling`, optimistic CAS) and queues the teardown job in one commit; simultaneous requests share one job (`started: false` for the loser); the row keeps its identity until the fenced worker has verified the Harness is gone and the role deleted (per-step `progress` on the job); a failed teardown gets attempt N+1; `409 agent.deploy_in_progress` while a deploy runs |
