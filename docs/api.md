...
| — | *multi-actor / multi-session scenarios* | A predefined scenario may carry `metadata.launchpad_execution` (`{version: 1, repeat 1–5, steps[{turn, actor, session}], checks[{id, type: exact\|contains\|not_contains, turn, text, depends_on?, case_sensitive?}]}` — see [architecture](architecture.md#executable-multi-actor--multi-session-evaluation-scenarios)). Create / upload / update validate it strictly: malformed or out-of-bounds blocks, unknown keys, duplicate/missing/out-of-range turns, bad aliases, persona mixing or a top-level key → 422 `dataset.invalid_execution`; a dataset expanding past 200 invocations or 500 sessions → 422 `dataset.execution_limits`. The block is stored and synced verbatim; a `PUT` with the block keeps it |
...
| `GET` | `/api/eval/runs/{run_id}` | One run (scores / insight trees / `batch_eval_id` / `error` / `stop_requested`). Runs whose dataset opted into `metadata.launchpad_execution` also carry `execution` — `{version, scenarios[{scenario_id, repeat, steps, checks}], calls_planned, calls_done, sessions[{scenario_id, repeat, actor, actor_id, session, requested_session_id, session_id, drift, turns[], status: ok\|partial\|failed}], steps[{…, status, error?, response_excerpt?}], checks[{scenario_id, repeat, id, type, turn, text, outcome: pass\|fail\|error\|inconclusive, evidence}], check_status: pending\|none\|pass\|fail\|error\|inconclusive}` — LOCAL deterministic results over the agent's actual replies (not AWS judge scores), re-persisted after every step so stopped/failed runs still list every session opened; `null` otherwise |
