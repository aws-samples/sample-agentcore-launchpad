...
created_at, updated_at, running}`. A `lambda_function` result carries `revision_id` (the
...
the append-only audit entries — no CloudTrail actor or token is ever stored or returned. `POST /api/eval/runs` answers
...
| — | *multi-actor / multi-session scenarios* | A predefined scenario may carry `metadata.launchpad_execution` (`{version: 1, repeat 1–5, steps[{turn, actor, session}], checks[{id, type: exact\|contains\|not_contains, turn, text, depends_on?, case_sensitive?}]}` — see [architecture](architecture.md#executable-multi-actor--multi-session-evaluation-scenarios)). Create / upload / update validate it strictly: malformed or out-of-bounds blocks, unknown keys, non-integer `version`, duplicate/missing/out-of-range turns, bad aliases, persona mixing, a misplaced key (top level / non-object `metadata`), an opt-in item over 16 000 serialized characters, or a normalized scenario-id collision (legacy prompt items are `item_<N>`) → 422 `dataset.invalid_execution`; a dataset expanding past 200 invocations or 500 sessions → 422 `dataset.execution_limits`. These gates apply only to datasets containing an opt-in item. The block is stored and synced verbatim; a `PUT` with the block keeps it |
...
| `GET` | `/api/eval/runs/{run_id}` | One run (scores / insight trees / `batch_eval_id` / `error` / `stop_requested`). Runs whose dataset opted into `metadata.launchpad_execution` also carry `execution` — `{version, scenarios[{scenario_id, repeat, steps, checks}], calls_planned, calls_done, sessions[{scenario_id, repeat, actor, actor_id, session, requested_session_id, session_id, returned_session_id?, drift, turns[], status: ok\|partial\|failed}], steps[{…, status: ok\|failed\|empty, error?, response_excerpt?}], checks[{scenario_id, repeat, id, type, turn, text, outcome: pass\|fail\|error\|inconclusive, evidence}], check_status: pending\|none\|pass\|fail\|error\|inconclusive, interrupted}` — LOCAL deterministic results over the agent's actual replies (not AWS judge scores), re-persisted after every step so stopped/failed runs still list every session opened; `null` otherwise |
...
| `POST` | `/api/eval/runs` | Start a run (exactly one scope: `dataset_id` \| `cloud_dataset_id` \| `session_ids` \| `lookback_hours`) → 201. A `cloud_dataset_id` scope may add `dataset_version` (a published version number such as `"2"`, never `DRAFT`; omitted = the draft): the version must exist in `ListDatasetVersions` (else 422 `run.dataset_version_unknown`, no run row) and `GetDataset` / `ListDatasetExamples` read that snapshot. `dataset_version` with any other scope → 422 `run.dataset_version_scope`. Every run row echoes `dataset_version` (`null` for draft, local, session and window runs). A dataset with `metadata.launchpad_execution` scenarios against an A2A agent → 422 `run.execution_unsupported_protocol` (no actor envelope), and local as well as cloud items are re-validated (`dataset.invalid_execution` / `dataset.execution_limits`) — all before a run row exists |
