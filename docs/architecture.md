...
## Executable multi-actor / multi-session evaluation scenarios

An ordinary predefined scenario replays its `turns` sequentially in **one** runtime
session under the bare `default` actor, which cannot exercise memory-isolation or
session-freshness golden tests ("A tells the agent a secret in one session, B must not
learn it", "A opens a new session and the agent must still remember"). A scenario may
**opt in** to a procedure runner through a versioned, namespaced block inside its
standard `metadata` — `metadata.launchpad_execution` — which the SDK's `Scenario.metadata`
and the AWS dataset schema (free-form example documents) both preserve. Everything
without the key keeps the old wire behaviour exactly; simulated personas, legacy prompt
items and A2A agents cannot opt in (`backend/app/evaluation/execution.py`).
...
so the per-agent memory partitioning and the Observability actor probes apply — and one fresh
...
the run row's `execution` JSON and `session_ids` are re-persisted — a stopped or failed run
still lists every runtime session it opened. Ground truth is split by the **actual** session
rather than the legacy positional zip: each session's `sessionMetadata` entry carries the
`expectedResponse` turns it replayed, a unique `testScenarioId`
(`<scenario>#r<repeat>/<session alias>`) and the correlation as the API's string-map
`metadata` (`launchpad_scenario/repeat/actor/session`); scenario-level `assertions` /
`expected_trajectory` attach only to the session that carried the last step (the outcome
session), so a negative-isolation assertion is never judged against the seeding actor's
session. Plain scenarios in the same dataset keep their exact legacy call shape and order.
...
| `eval_datasets` / `eval_runs` | Evaluation datasets (legacy prompts or devguide scenarios + description + last AWS-sync blob) and run state (scores or insight trees; window runs encode their scope as `dataset_name="window:<N>h"`; opt-in multi-actor/multi-session runs add the `execution` ledger — minted sessions with their synthetic actors, steps and local check outcomes) |
