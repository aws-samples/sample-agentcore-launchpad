...
# 第 06 章 · 可观测性（Transaction Search · 追踪 · Token 与成本）
...
![可观测仪表盘](/static/images/06-obs-dashboard.png)
*图 6-1：仪表盘界面示例。请在覆盖自己测试流量的时间窗口内读取数据。*
...
| 会话 | 确认 `lab-quota-advisor` 的测试会话已出现 |
...
```
lab-quota-kb-<kb_id 小写>___Retrieve    <N> 次调用
skills                                  <N> 次调用
```
...
![会话列表](/static/images/06-obs-sessions.png)
*图 6-2：业务会话列表示例。每行显示追踪数、LLM 调用数、token、估算成本、起止时间与错误数。*

会话列表中的字段结构如下：

```
<SESSION_ID_A>  lab-quota-advisor  <TRACE_COUNT> traces  <LLM_COUNT> LLM  <TOKENS> tokens  <EST_COST>
<SESSION_ID_B>  lab-quota-advisor    <TRACE_COUNT> traces  <LLM_COUNT> LLM  <TOKENS> tokens  <EST_COST>
```

在相同时间范围内比较不同会话的 LLM 调用数、token 和成本。检索结果和工具循环会扩大上下文，
因此触发知识库检索的会话往往消耗更多 token；具体差值以自己的会话为准。

点开一个会话进入详情：

![会话详情](/static/images/06-obs-session-detail.png)
*图 6-3：advisor 会话详情示例。上半从 AgentCore Memory 还原对话，下半列出会话内追踪，
并可回跳到对话演练场。*
...
## 6.4 追踪视图：技术视角（span 瀑布图）

点任意一条「会话内追踪」卡片，或切到 `追踪` 标签选一条 trace。

![追踪瀑布图](/static/images/06-obs-trace.png)
*图 6-4：一条知识库检索追踪的瀑布图示例。span 数、LLM 调用数和耗时以自己的页面为准。*

选择第 05 章查询 Runtime 部署大小或区域会话配额的 trace。记录 trace ID、模型、根 span 耗时、
...
└─ invoke_agent Strands Agents                              <AGENT_DURATION>
   ...
   ├─ execute_tool lab-quota-kb-…___Retrieve                <RETRIEVAL_DURATION>
   └─ chat <SELECTED_MODEL_ID>                              <LLM_DURATION>
```
...
> `AgenticRetrieveStream`，对明确的英文配额名使用单知识库 `Retrieve`。
> 第 04 章填写的 KB 描述会帮助 Agent 选择检索方式。

点某个 span 打开右侧抽屉：

![span 抽屉](/static/images/06-obs-span-drawer.png)
*图 6-5：`chat` span 抽屉示例。请从自己的调用读取输入/输出 token、预估成本和首 token 延迟。*

抽屉底部的「原始属性」是 span 上的原始 OTel 属性，重点查看：

```
"aws.local.service":  "harness_lab_quota_advisor.DEFAULT"
...
```
## Knowledge bases
Retrieval tools are mounted for you. Prefer `agentic-lab-quota-advisor___AgenticRetrieveStream`
...
- lab-quota-kb (tool `lab-quota-kb-<kb_id 小写>___Retrieve`) — Amazon Bedrock AgentCore 服务配额资料…
...
`service.name`（如 `lab_quota_assistant_<后缀>.DEFAULT`、`harness_lab_quota_advisor.DEFAULT`）
会通过台账映射回平台 Agent 名显示。所以列表里看到的是 `lab-quota-advisor`，而 span 的
原始属性里是 `harness_lab_quota_advisor.DEFAULT`。
...
- [ ] 「热门工具」里出现自己的 `lab-quota-kb-<kb_id 小写>___Retrieve`
- [ ] 会话列表能看到自己的 `lab-quota-advisor` 会话
...
基础部分（Part 1）到此完成。只做基础部分的参与者可直接进入
[第 10 章 · 收尾与资源清理](../10-wrapup-cleanup)；继续进阶部分（Part 2）则进入下一章。

上一章：[第 05 章 · 对话测试与记忆](../05-chat-memory) ｜
下一章：[第 07 章 · 评估](../07-evaluation)
