...
> **目标**：核对挂载知识库后的回答是否有资料依据，确认技能生效，并查看短期事件、长期记录及其分区。
>
...
本章只操作 `lab-quota-advisor`。重点是核对回答有没有资料依据，以及对话内容如何落进
AgentCore Memory。
...
后面 5.5 与 5.6 要在 Memory 控制台里读回本章的对话，先用三轮对话写入一组可核对的偏好。
仍然使用 `lab-quota-advisor`。
...
2. 第二轮换一个话题，检查偏好是否被保留：

```
我们准备上线 AgentCore Runtime。请用不超过 6 行的表格列出需要优先核对的容量边界，并简要说明原因。
```

![多轮对话](/static/images/05-chat-assistant.png)
*图 5-2：第二轮回答保留了中文、表格和可调整性标注。*
...
三轮都要核对回答是否保留中文、表格、区域和可调整性这几项偏好。第三轮的数值与配额文档比对：
`us-east-1` 为 5,000，`ap-southeast-1` 为 2,500。声明过的偏好在后续轮次失效，属于短期记忆
没有生效，5.5 可以据此排查。
...
- 面板底部还给出等价 API 调用（`curl -N … /v1/agents/<id>/invoke-stream`），可直接复制到终端执行。
...
选择 `lab-quota-advisor · <USER_NAME>`，并确认 actor 符合
`<agent_id>__<USER_NAME>` 格式。

1. 点 `lab-quota-advisor · <USER_NAME>`
...
切到 `长期记忆` 标签，选参与者 `lab-quota-advisor · <USER_NAME>`，策略选
`user_preferences`。
...
| 追踪面板长时间「暂无 SPAN」 | Transaction Search 摄取延迟约 1 分钟起 | 等一会点 `⟳ 加载链路`；第 06 章统一看 |
| 重新发布后行为没变 | 已有会话被钉在旧版本 | 点 `新会话` 再验证 |

---

上一章：[第 04 章 · 挂载能力](../04-capabilities) ｜
下一章：[第 06 章 · 可观测性](../06-observability)
