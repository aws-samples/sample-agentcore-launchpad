...
>
> **前置条件**：完成[第 05 章](../05-chat-memory)，`lab-earnings-assistant` 可正常对话。
>
> **预计耗时**：约 10 分钟。
>
...
> 截图或录屏演示时注意：新建那一刻的明文会完整显示在界面上，不要录进材料。备注名由界面
> 自动生成（`console-1`、`console-2` …）；停用有确认框，确认后使用该密钥的客户端立即收到
> 401，之后可重新启用。
...
> `<ASSISTANT_ID>` 从这一页右下角的「等价 API 调用」面板里取：选中 `lab-earnings-assistant` 后，
...
curl -s -X POST "http://127.0.0.1:8000/v1/agents/$AGENT_ID/invoke" \
  ...
  -d '{"prompt":"写一份季度财报点评通常需要核对哪些关键指标？用三行以内回答。","session_id":null}' | python3 -m json.tool
```
...
curl -sN -X POST "http://127.0.0.1:8000/v1/agents/$AGENT_ID/invoke-stream" \
  -H "x-api-key: $LP_KEY" -H 'content-type: application/json' \
  -d '{"prompt":"用两句话说明为什么财报点评要区分经营利润和非经营性投资收益。","session_id":null}'
```

`lab-earnings-assistant` 是 ZIP Runtime，流式接口会原生逐字输出，并在触发工具时给出 `tool`
事件。下面保留事件结构，文本和耗时以你的调用结果为准：

```
event: meta
data: {"session_id": "<session id>", "agent": "lab-earnings-assistant", "mode": "stream"}
...
| `404` agent not found | agent id 写错，或 Agent 不是 active | 在 `02 Agent 管理` 确认状态是 `运行中`，再从「等价 API 调用」面板重新取 id |
| 流式没有逐字输出、短时间内集中吐出 | ZIP Runtime 使用缓冲兼容路径 | 属预期；托管 Harness 支持原生增量流式输出 |
| `curl` 一直挂着不返回 | 忘了 `-N`，或模型生成较慢 | 加 `-N` 关闭缓冲；模型生成可能需要数秒 |
| 连接错误（8000 或 5173） | 后端/前端未启动，或使用了自定义端口 | 查看 `./start.py` 输出中的实际端口和 `.run/` 日志 |
