...
第01章 环境          bootstrap → 共享基础设施 + AgentCore 单例
   ↓
第02章 部署          Strands ZIP → lab-earnings-assistant
第03章 部署          托管 Harness → lab-earnings-advisor
   ↓
第04章 挂能力        Q2 2026 财报 PDF → 托管知识库；技能 → Registry(DRAFT→提交→批准) → 挂到 Harness
   ↓
第05章 测试          对话验证：有 KB 的检索作答，无 KB 的必然拒答/编造；记忆分区与长期偏好抽取
...
| 资源 | 标识 | 清理方式 |
|---|---|---|
| 本地数据集 `lab-earnings-dataset` | `<DATASET_ID>` | `?view=datasets` 选中 → `删除` |
| 本地 A/B 数据集 `lab-earnings-audit-ab`（第 09 章） | `<DATASET_ID>` | 同上；实验的 `CLEANUP` 不会删除数据集 |
| AWS 数据集快照 | `dataset/lab_earnings_dataset-<后缀>` | 云端副本不可编辑，只能删除 |
| 自定义评估器 `earnings_fact_grounding` | `earnings_fact_grounding-<后缀>` | `?view=evaluators` 选中 → `删除` |
| 自定义评估器 `claim_verdict_consistency`、`claim_verdict_vocabulary`（第 09 章） | 各自 `<名称>-<后缀>` | 同上；实验的 `CLEANUP` 不会删除评估器 |
| 评估运行记录（第 08 章两次 + 第 09 章一次） | `run-<后缀>` | 本地台账记录，留着做对比即可 |
| 回放产生的 Runtime 会话与遥测 | 每次运行 5 个 session | CloudWatch 日志按保留期自动过期 |
...
6. 删除本地数据集 lab-earnings-dataset；完成第 09 章时一并删除 lab-earnings-audit-ab；再删除云端快照与三个自定义评估器
...
| 来源 | 参考量级 |
|---|---|
| Nova 2 Lite 模型调用（对话 + 评估回放 + 评审 + A/B 流量 + 优化器） | 两个 Agent 均使用同一轻量模型；完整流程为数十次调用 |
...
- 想改这套实验做自己的 demo：替换第 04 章的财报文档（任何一份晚于模型训练截止的资料都能
  复现「无知识库必然无依据」的对比），并同步重写第 08 章的数据集真值和第 09 章的核对题。
...
| 知识库删除失败 | Harness 或转换后的 Runtime 仍引用该 KB | 先删除 `lab-earnings-advisor-rt`，再编辑 `lab-earnings-advisor` 取消知识库并重新发布，最后删除 KB |
