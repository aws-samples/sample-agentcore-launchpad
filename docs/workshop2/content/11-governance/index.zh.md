...
> 跳过后可直接进入[第 12 章](../12-wrapup-cleanup)。本章继续使用 bootstrap 提供的真实
> `hr-database` 与 `office-facts` Gateway 工具，不把它们改名为财报工具。
>
> **目标**：纳管一个 Gateway，创建 `LOG_ONLY` Cedar 策略，造出真实的 ALLOW / DENY 判定，
> 再从决策视图与审计日志里把证据读出来。
>
> **前置条件**：完成第 04 章。第 01 章 bootstrap summary 中 `gateway traces` 应为
> `created` 或 `present`；否则策略指标仍能工作，但逐条 Policy span 明细不可用。
>
...
> （策略创建需要，留到第 12 章清理）。11.6 会对 Gateway 发起几次真实工具调用，
> 产生 CloudWatch 策略指标与 Policy span（只读遥测，无需清理）。
>
> **注意**：本章操作的是共享资源（`launchpad-gw` 及其策略引擎）。新建策略一律先
> `LOG_ONLY`，不要在实验环境里直接把某个 Gateway 切到 `ENFORCE`。
...
完成前序章节后，当前账号至少有下面两个 Launchpad 网关，两个都应显示为「未纳管」：
...
> `launchpad-kb-gw` 是第 04 章挂知识库时平台自动创建的连接器网关，目标数随挂载的 KB
> 变化。清单不含实验专属网关：第 09 章的 `launchpad-exp-gw` 和第 10 章的 `lp-canary-*`
> 由平台自管，治理清单会过滤掉。自有账号里可能还有其它历史网关，本章只操作上面两行。

## 11.2 只读打开一个 Gateway

点击任意一行进入详情。只是打开，不会写任何东西。
...
- **Registry 发布**：`GATEWAY 记录 未入目录`；`旧记录` 数量取决于账号内是否已有按目标记录
...
- **IAM 挂载预检**：平台调用 `iam:SimulatePrincipalPolicy` 检查 Gateway 执行角色。
  `PASS` 表示权限齐全；`FAIL` 时按界面给出的内联策略 JSON 补齐；当前操作身份不能执行
  模拟时显示 `UNKNOWN · simulation_denied`

## 11.3 纳管：只加两个标签，不动任何资源

纳管状态由两个 durable 标签标记，除此之外不修改资源的任何配置：
...
*图 11-3：未纳管状态。策略引擎显示 `未挂载`，并提供 `创建并以 LOG_ONLY 挂载` 按钮。*

2. 点击 `纳管`，在确认弹窗里确认。
3. 到 AWS 侧核对，这一步是本节的重点：
...
4. 点击 `取消纳管` 并确认，再查一次标签，应返回 `{"tags": {}}`。

取消纳管只移除这两个标签，不解绑也不删除引擎、策略或 Registry 记录。反过来，Registry
导入和 Policy 变更都要求 Gateway 带着这两个标签，并校验最新 `updatedAt`。

### 再纳管 `launchpad-gw`：11.5 的前置条件

`launchpad-gw` 未纳管时 `新建策略` 是禁用的，策略编辑器只读。进 11.5 之前回到
`GATEWAY 列表` 打开 `launchpad-gw`，点击 `纳管` → 确认，**这次不要取消**。
第 12 章清理时再取消纳管，已创建的策略不受影响。

## 11.4 Registry 边界：一个 Gateway = 一条 MCP 记录

Gateway 详情的「REGISTRY 发布」区有 `预览` 与 `导入 GATEWAY`。点击 `预览`（只读），
...
> 工具目录放在 `inlineContent` 里，是一段被转义的 JSON 字符串。顺着读能数出工具名，
> 形如 `hr-database___check_calendar`，即 `<target>___<tool>`。11.5 写策略时使用这个命名。

两条边界规则：

1. **Registry 审批 ≠ 授权**。审批只决定记录在目录里可见、可挂载；挂载一条 Gateway 记录
   等于把整个 Gateway 及其全部工具给了 Harness，按动作的授权只能靠 Cedar 策略。
2. 导入 Gateway 记录后，已有的按目标旧记录仍然存在，要显式点击 `停用选中的旧记录` 才
   退休；`DEPRECATED` 是终态。

> 本章只跑预览，不执行导入。导入按钮的行为与预览返回的 `proposed` 完全一致。

## 11.5 Cedar 策略编辑器：创建一条 `LOG_ONLY` 策略

切到 `策略编辑器` 标签（也可以从 Gateway 详情点击 `新建策略`）。前提是 `launchpad-gw`
已按 11.3 末尾纳管。

![策略编辑器](../static/images/11-policy-editor.png)
*图 11-5：策略编辑器。三种授权模型，下面是从 Gateway 实时发现的精确动作列表。图中五个
只读动作已勾选，`hr-database___create_payout` 保持未勾选；此刻 CEDAR 还是空的，
发布按钮上方写着 `暂时无法保存：请生成或粘贴 Cedar 草稿`。*
...
*图 11-6：草稿生成后填入 `CEDAR` 框，发布按钮上的阻塞提示消失。下方「CEDAR 审核」区把
`AWS 实时版本` 与 `草稿` 并排对比。*

「策略发布」区显示三个事实：`GATEWAY 模式 ENFORCE`、`策略模式 LOG_ONLY`，以及 24 小时
窗口内的 `AWS 证据` 计数（按章节顺序操作的账号此时为 `0 / 24h`）。

`输入 GATEWAY 名称` 与 `零证据覆盖原因` 两个输入框保存草稿时都不用填（界面标注了适用
范围，真正的硬门禁在提升到 `ENFORCE` 那一步）。留空，直接点击 `创建 LOG_ONLY 策略`，
再在弹窗中确认。

![发布确认](../static/images/11-policy-confirm.png)
*图 11-7：发布确认。弹窗标题「保存策略」，正文「为 launchpad-gw 以 LOG_ONLY 保存
`lab_readonly_tools`」。*
...
> **`status` 与 `enforcement_mode` 不是一回事**：`status: ACTIVE` 只说明策略资源存在且
> 可用，决定会不会拦的是 `enforcement_mode`。Gateway 另有自己的挂载模式（本例 `ENFORCE`），
> 两者相互独立。刚提交时 `status` 会短暂停在 `CREATING`，十几秒后转 `ACTIVE`。

### 生命周期与提升门禁

- 新引擎、新 Gateway 挂载、新策略一律从 `LOG_ONLY` 开始。
- 编辑一条执行中的策略不会就地改它，而是创建一个 `LOG_ONLY` 候选；提升与回滚走保守顺序，
  先让候选生效，再退役旧的。
- **把 Gateway 切到 `ENFORCE` 需要证据**：界面要求 24 小时内有 LOG_ONLY 决策证据，
  没有证据就要手输 Gateway 名称并填写零证据覆盖原因。

## 11.6 决策证据：自己造一次 ALLOW 和 DENY

账号里还没有工具调用时决策视图是空的。平台提供了策略测试接口：以指定身份对 Gateway
发起一次真实的 `tools/call`，把判定结果记进决策台账。两个内置身份的权限不同：
...
命令应返回一条 `DENY`，记录响应中的 `decision_id` 和实际策略 ID：
...
- 想多凑几条证据，换几个只读工具各打一次，如 `hr-database___list_departments`、
  `office-facts___get_office_fact`。

### 回到决策视图读证据

切到 `决策` 标签（要先在 `GATEWAY 列表` 里选中一个 Gateway）。证据存在摄取延迟：
CloudWatch 策略指标约 2–5 分钟；逐条明细走 Policy span，通道正常时约十几秒。
未出现时点击 `刷新`。

逐条明细还有一个独立前置条件：当前 Gateway 必须启用 tracing，平台会实时检查并给出警告：

- 没有警告：通道为 `ready`，继续等待本节刚产生的 span。
- 「Gateway tracing 尚未配置」：运行 `make bootstrap` 确认 summary 为
  `gateway traces created/present`，然后**重新执行** `policy-test`。Tracing 不会补写启用前
  的调用。
- 「无法确认 Gateway tracing 配置」：按提示检查 bootstrap 身份是否具备第 01 章列出的六个
  CloudWatch Logs delivery 权限。

![决策](../static/images/11-decisions.png)
*图 11-8：决策视图。上半是 CloudWatch 策略指标的汇总与三种拆分，下半是来自 Policy span
的逐条明细，可按时间范围与具体策略过滤。*

汇总区读取并记录自己页面中的总数和各分组计数：
...
1. **拦截带着理由**。DENY 行直接写出是哪条策略拦的，可以照着策略名反查 Cedar 语句。
2. **`LOG_ONLY 本会匹配` 就是影子模式的价值**。ALLOW 行真正生效的是
   `launchpad_baseline_allow`，界面同时标注 `lab_readonly_tools` 若已在执行也会匹配这类
   调用，不影响现网就能先看清它将会怎么判。DENY 行没有这个标注，因为 `create_payout`
   不在白名单里。
3. **主体列显示 `span 中没有`**。判定主体不在 Policy span 的字段里。要知道是谁调的，
   用 `policy-test` 返回里的 `principal`，或顺着 `TRACE` 列去第 07 章的追踪视图。

> 「其中 … 条为 LOG_ONLY」指由 LOG_ONLY 策略做出的判定数。本节调用由 `ACTIVE` 策略判定，
> `lab_readonly_tools` 只显示「本会匹配」，并未参与实际判定，所以 LOG_ONLY 计数和 11.5 的
> `AWS 证据 … / 24h` 不一定增加。
...
*图 11-9：策略变更审计。左侧每次变更一行，点开后右侧是完整快照：`变更前`、`请求内容` 与
`AWS 结果` 三段 JSON。*

找到本章创建策略留下的记录，核对时间、引擎 ID、操作人和状态：

```
<TIME> | policy_create | <POLICY_ENGINE_ID> | <OPERATOR> | succeeded
```

`请求内容` 里能看到这次保存的完整意图，包括乐观锁与两个门禁字段的实际取值：
...
> `覆盖原因` 显示 `-`，因为 11.5 保存草稿时没填（`override_reason: null`）。要让别人日后
> 看懂一条 LOG_ONLY 策略为什么存在，策略的 `描述` 加这里的覆盖原因是唯一线索。
> `expected_gateway_updated_at` 是乐观锁：与 AWS 侧最新 `updatedAt` 不一致就拒绝写入。
...
- [ ] `launchpad-kb-gw` 纳管后 AWS 侧只多两个标签，取消纳管后标签为空
...
| 现象 | 原因 | 处理 |
|---|---|---|
| `创建 LOG_ONLY 策略` 不可点 | 还没有 Cedar 草稿 | 先点击 `生成 CEDAR 草稿`；保存草稿不需要填 Gateway 名称与覆盖原因 |
| `新建策略` 是灰的 / 编辑器提示「未纳管，只读」 | `launchpad-gw` 没有 launchpad 标签 | 按 11.3 末尾先纳管它 |
| 报 `updatedAt` 相关冲突 | 平台要求变更前 Gateway 状态新鲜 | 点击 `刷新` 后重试 |
| 汇总与明细都是 0 条 | 账号内还没有任何 Gateway 工具调用 | 按 11.6 用 `policy-test` 调用几次，指标 2–5 分钟后到达，再点击 `刷新` |
| 汇总有计数、明细持续为空 | trace 投递未配置，或刚创建尚未生效 | 确认 `gateway traces` 为 `created` 或 `present`，等待数分钟后重新执行 `policy-test` |
| `policy-test` 返回 `ERROR`、`recorded: false` | 评估根本没发生（凭证问题、Gateway 不可达等） | 错误不是判定，不入账；先确认 Gateway 是 `READY` 再重试 |
