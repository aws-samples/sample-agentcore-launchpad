...
> 但 Nova 2 Lite 把财报原文的 $42,232 million 换算成了「4,223 亿美元」——差了 10 倍。
> 检索是对的、编造并不存在，错的是换算这一步。把这类观察记下来：第 08 章的
> `earnings_fact_grounding` 会按「单位与量级」扣它的分，第 09 章的核对纪律会正面修它。
...
英文指标名缩小检索范围。重点检查两处：**方向（净流出）**有没有说对——把负值说成正值是
财报问答里最危险的错误之一；以及**亿美元换算**是否又差了量级（实测中 $7,604 million
被写成「7,604 亿美元」，方向和归因都对，量级错了 100 倍）。
...
的数值。实测中 Nova 2 Lite 的典型回答是**「2026 年第二季度的财务数据尚未发布，因此目前
无法提供具体数值」**——在它的训练数据里这份财报确实还不存在，这句话本身就是结构性坏例
最直观的证据。如果它给出数值，与财报中的 `净销售额 2,006 亿美元`、`AWS 422 亿美元` 比对。
...
- [ ] 已记录 advisor 的换算或口径失误（若有）——第 08、09 章会分别量化和修复它们
