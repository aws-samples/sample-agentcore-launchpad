...
*图 10-1：当前环境的金丝雀创建页已选中 `lab-earnings-assistant · zip_runtime`，
...
> 实验环境没有业务流量，使用 `lab-earnings-dataset (5)` 播种测试调用。
...
复核生产状态：回到 `02 Agent 管理`，`lab-earnings-assistant` 应仍是 `运行中`；点这一行的
`编辑`，系统提示词应该已经回到原始版本，没有候选版本里那段「回答原则」：

```text
你是投资研究团队的亚马逊财报分析助手。帮助分析师解读 Amazon 2026 年第二季度业绩，梳理收入、
利润、现金流、分部业绩和业绩指引；不确定具体财务数据时明确说明需要查阅官方财报，不要猜测数值。
```
...
> 把生产行为恢复成当前版本的配置并停掉 A/B。Agent 的系统提示词回到了没有「回答原则」的
