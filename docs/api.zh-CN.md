...
| `GET` | `/api/eval/runs/{run_id}` | 单个运行(分数 / insight 树 / `batch_eval_id` / `error` / `stop_requested`)。数据集启用了 `metadata.launchpad_execution` 的运行还带 `execution` —— `{version, scenarios[{scenario_id, repeat, steps, checks}], calls_planned, calls_done, sessions[{scenario_id, repeat, actor, actor_id, session, requested_session_id, session_id, drift, turns[], status: ok\|partial\|failed}], steps[{…, status, error?, response_excerpt?}], checks[{scenario_id, repeat, id, type, turn, text, outcome: pass\|fail\|error\|inconclusive, evidence}], check_status: pending\|none\|pass\|fail\|error\|inconclusive}`，即基于 agent 实际回复的**本地**确定性结果（不是 AWS 评审得分），每一步之后重新持久化，因此被停止/失败的运行同样列出打开过的每个会话；否则为 `null` |
