...
| `GET` | `/api/assistant/architect/conversations` | 成员 | `{conversations[{id, title, turns, turn_in_progress, status, proposal_status, proposal_revision, created_at, updated_at}]}`——仅调用者自己的会话，最新在前（≤ 50） |
...
| — | *多角色 / 多会话 scenario* | predefined scenario 可携带 `metadata.launchpad_execution`（`{version: 1, repeat 1–5, steps[{turn, actor, session}], checks[{id, type: exact\|contains\|not_contains, turn, text, depends_on?, case_sensitive?}]}`，见[架构文档](architecture.zh-CN.md)对应专节）。创建 / 上传 / 更新都严格校验：块格式错误或越界、未知键、非整数 `version`、重复/缺失/越界的轮次、非法别名、与 persona 混用、位置错误的键（顶层 / 非对象 `metadata`）、启用条目序列化超过 16 000 字符、或归一化 scenario id 碰撞（旧式 prompt 条目为 `item_<N>`）→ 422 `dataset.invalid_execution`；数据集展开超过 200 次调用或 500 个会话 → 422 `dataset.execution_limits`。这些门槛只作用于包含启用条目的数据集。该块原样存储并同步；带块的 `PUT` 会保留它 |
...
| `GET` | `/api/eval/runs/{run_id}` | 单个运行(分数 / insight 树 / `batch_eval_id` / `error` / `stop_requested`)。数据集启用了 `metadata.launchpad_execution` 的运行还带 `execution` —— `{version, scenarios[{scenario_id, repeat, steps, checks}], calls_planned, calls_done, sessions[{scenario_id, repeat, actor, actor_id, session, requested_session_id, session_id, returned_session_id?, drift, turns[], status: ok\|partial\|failed}], steps[{…, status: ok\|failed\|empty, error?, response_excerpt?}], checks[{scenario_id, repeat, id, type, turn, text, outcome: pass\|fail\|error\|inconclusive, evidence}], check_status: pending\|none\|pass\|fail\|error\|inconclusive, interrupted}`，即基于 agent 实际回复的**本地**确定性结果（不是 AWS 评审得分），每一步之后重新持久化，因此被停止/失败的运行同样列出打开过的每个会话；否则为 `null` |
