...
| [Managed Knowledge Bases](./managed-kb.md) | Managed KB CRUD + S3 sources + Playground; launchpad-kb-gw connector topology (per-KB Retrieve + per-agent AgenticRetrieveStream targets) for harness attach; direct bedrock:Retrieve `kb_search` channel for zip/container attach, kb-role + exec-role IAM, async create/ingest quirks | Active |
